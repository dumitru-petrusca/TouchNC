...
function on_macro_filename(item, index) {
    ...
    if (filename.length == 0) {
        alertdlg(translate("Out of range"), translate("File name cannot be empty!"));
    }
...
function closeMacroDialog() {
    ...
    if (modified) {
        confirmdlg(translate("Data mofified"), translate("Do you want to save?"), process_macroCloseDialog)
...
function macroUploadfailed(errorcode, response) {
    alertdlg(translate("Error"), translate("Save macro list failed!"));
    displayNone('macrodlg_upload_msg');
}
