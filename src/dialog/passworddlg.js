...
function checkpassword() {
    ...
    id('password_content').innerHTML = "";
    displayNone('change_password_btn');
    if (pwd1 != pwd2) id('password_content').innerHTML = translate("Passwords do not matches!");
    else if (pwd1.length < 1 || pwd1.length > 16 || pwd1.indexOf(" ") > -1) id('password_content').innerHTML = translate("Password must be >1 and <16 without space!");
...
function ChangePasswordfailed(errorcode, response_text) {
    ...
    if (typeof(response.status) !== 'undefined') id('password_content').innerHTML = translate(response.status);
