...
function logindlg(closefunc, check_first) {
    ...
    id('login_title').innerHTML = translate("Identification requested");
...
function checkloginsuccess(response_text) {
    ...
    if (typeof(response.authentication_lvl) !== 'undefined') {
        if (response.authentication_lvl != "guest") {
            if (typeof(response.authentication_lvl) !== 'undefined') id('current_auth_level').innerHTML = "(" + translate(response.authentication_lvl) + ")";
...
function loginfailed(errorcode, response_text) {
    var response = JSON.parse(response_text);
    if (typeof(response.status) !== 'undefined') id('login_title').innerHTML = translate(response.status);
    else id('login_title').innerHTML = translate("Identification invalid!");
    console.log("Error " + errorcode + " : " + response_text);
    displayBlock('login_content');
    displayNone('login_loader');
    id('current_ID').innerHTML = translate("guest");
