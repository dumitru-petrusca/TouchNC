let EP_HOSTNAME = "Hostname";
let EP_STA_SSID = "Sta/SSID";
let EP_STA_PASSWORD = "Sta/Password";
let EP_STA_IP_MODE = "Sta/IPMode";
let EP_STA_IP_VALUE = "Sta/IP";
let EP_STA_GW_VALUE = "Sta/Gateway";
let EP_STA_MK_VALUE = "Sta/Netmask";
let EP_WIFI_MODE = "WiFi/Mode";
let EP_AP_SSID = "AP/SSID";
let EP_AP_PASSWORD = "AP/Password";
let EP_AP_IP_VALUE = "AP/IP";
let SETTINGS_AP_MODE = 2;
let SETTINGS_STA_MODE = 1;
...
function heading(label) {
    return "<h4>" + translate(label) + "</h4><hr>";
}
function item(label, pos, extra) {
    return translate(label) + table(build_control_from_pos(pos, extra));
}
...
function setupdlg() {
    ...
    id("wizard_button").innerHTML = translate("Start setup");
...
function enablestep1() {
    ...
    id("wizard_button").innerHTML = translate("Continue");
...
function enablestep2() {
    ...
    content += translate("AP define access point / STA allows to join existing network") + "<br>";
    content += spacer();

    content += startDiv("setup_STA");

    content += item("What access point ESP need to be connected to:", EP_STA_SSID);
    content += translate("You can use scan button, to list available access points.") + "<br>";
...
function enablestep4() {
    closeStep("step3link");
    id("wizard_button").innerHTML = translate("Close");
